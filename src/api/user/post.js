...
// GET /api/usr/post/like/users?postId={id} → [{ userId, userNm }]
export const GetLikeUsers = (postId) => {
    return api.get('/api/usr/post/like/users', { params: { postId } });
}
...
// GET /api/usr/post/product/{productId} → 특정 상품이 태그된 게시글 목록 (비로그인 가능)
export const ListPostByProduct = (productId, page = 0, size = 12) => {
    return api.get(`/api/usr/post/product/${productId}`, { params: { page, size } });
}
